...
int ch;
std::string saved_image_path = "/Users/mehdigorjian/Downloads/csce647/pr_08/reflection/out.ppm";
...
void render(const std::vector<Sphere> &spheres, const std::vector<Light> &lights) {
    ...
    for (int j = 0; j<height; j++) { // actual rendering loop
        for (int i = 0; i<width; i++) {
            ...
            Vec3f pix = cast_ray(Vec3f(0,0,0), Vec3f(dir_x, dir_y, dir_z).normalize(), spheres, lights);
            
            if (pix.x == BACKGROUND_COLOR.x && pix.y == BACKGROUND_COLOR.y && pix.z == BACKGROUND_COLOR.z)
            {
                framebuffer[i+j*width] = Vec3f(1, 1, 1);
            }
            else
            {
                framebuffer[i+j*width] = pix;
            }
        }
    }
    std::ofstream ofs; // save the framebuffer to file
    ofs.open(saved_image_path, std::ios::binary);
    ofs << "P6\n" << width << " " << height << "\n255\n";
