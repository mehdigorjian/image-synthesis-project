...
normal_map = pygame.image.load('img/normal_image.png')
...
c0_img = pygame.image.load('img/c0.png')
c0_img = pygame.transform.scale(c0_img, (W,H))
c1_img = pygame.image.load('img/c1.png')
c1_img = pygame.transform.scale(c1_img, (W,H))
env_img = pygame.image.load('img/env_image.png')
...
def processing():
    ...
    for i in range(W):
        for j in range(H):
            ...
            dx = (2 * compute_xyz(normal_map, i,j)[0] * compute_xyz(normal_map, i,j)[2] * d) / (2 * (compute_xyz(normal_map, i,j)[2])**2 - 1)
            dy = (2 * compute_xyz(normal_map, i,j)[1] * compute_xyz(normal_map, i,j)[2] * d) / (2 * (compute_xyz(normal_map, i,j)[2])**2 - 1)
            ...
            cr = (c0_img.get_at((i,j))[0]/255 * (1 - t) + c1_img.get_at((i,j))[0]/255 * t) * (1 - ks * s) + (env_img.get_at((i,j))[0]/255 * ks * s)
            cg = (c0_img.get_at((i,j))[1]/255 * (1 - t) + c1_img.get_at((i,j))[1]/255 * t) * (1 - ks * s) + (env_img.get_at((i,j))[1]/255 * ks * s)
            cb = (c0_img.get_at((i,j))[2]/255 * (1 - t) + c1_img.get_at((i,j))[2]/255 * t) * (1 - ks * s) + (env_img.get_at((i,j))[2]/255 * ks * s)
            # -------------------------------------------------
