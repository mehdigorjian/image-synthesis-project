...
Vec3f cast_ray(const Vec3f &orig, const Vec3f &dir, const std::vector<Sphere> &spheres, const std::vector<Light> &lights, size_t depth = 0)
{
    Vec3f point, N;
        ...
        float diffuse_light_intensity = 2.5, specular_light_intensity = 0;
        for (size_t i = 0; i < lights.size(); i++)
        {
            ...
            if (translucent_flag)
            {
                Vec3f reflect_dir = reflect(dir, N).normalize();
                Vec3f reflect_orig = reflect_dir * N < 0 ? point - N * 1e-3 : point + N * 1e-3;
                reflect_color = cast_ray(reflect_orig, reflect_dir, spheres, lights, depth + 1) * lights[i].intensity * 10;
            }
...
//#pragma omp parallel for
    for (size_t j = 0; j < height; j++)
    {
        for (size_t i = 0; i < width; i++)
        {
            ...
            framebuffer[i + j * width] = cast_ray(camera_origin, Vec3f(dir_x, dir_y, dir_z).normalize(), spheres, lights);
        }
    }
