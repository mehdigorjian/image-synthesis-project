...
def fresnel(k=1):
    while theta_deg <= 90:
        ...
        a = sqrt((sqrt((n**2-k**2-(sin(theta))**2)**2 + ((4 * n**2) * k**2)) + (n**2 - k**2 - (sin(theta))**2))/2)
        b = sqrt((sqrt((n**2-k**2-(sin(theta))**2)**2 + ((4 * n**2) * k**2)) - (n**2 - k**2 - (sin(theta))**2))/2)
        Fs = (a**2+b**2-(2 * a * cos(theta))+ (cos(theta))**2)/(a**2+b**2 + (2 * a * cos(theta))+(cos(theta))**2)
        Fp = Fs * ((a**2+b**2 - (2 * a * sin(theta) * tan(theta)) + (sin(theta))**2*(tan(theta))**2)/(a**2+b**2 + (2 * a * sin(theta) * tan(theta)) + (sin(theta))**2*(tan(theta))**2))
        R = (Fs + Fp)/2
    return R
...
def control():
    ...
    for i in range(W):
        for j in range(H):
            ...
            # b = (b - 0.9)/(0.95 - 0.9)
            # if (1 - b) < 0.2: f = (.2 -1 + getpixel_bound(n_img,i,j)[2])/.2
            # elif b < .6 : f = ((1-getpixel_bound(n_img,i,j)[2]) - .6)/(.8 - .2)
            # else: f = 1
            f = fresnel()
